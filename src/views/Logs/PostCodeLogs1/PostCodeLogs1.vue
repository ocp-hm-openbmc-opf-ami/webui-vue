  ...
  <b-container fluid="xl">
    ...
    <b-row class="align-items-start">
      <b-col sm="8" xl="6" class="d-sm-flex align-items-end mb-4">
        <search
          :placeholder="$t('pagePostCodeLogs1.table.searchLogs')"
    ...
    <b-row>
      <b-col xl="12" class="text-right">
        ...
        <b-button
          ...
          :href="href"
        >
          <icon-export /> {{ $t('pagePostCodeLogs1.button.exportAll') }}
        </b-button>
      </b-col>
    </b-row>
    <b-row>
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        ...
        {
          key: 'date',
          label: this.$t('pagePostCodeLogs1.table.created'),
          sortable: true,
        },
        {
          key: 'timeStampOffset',
          label: this.$t('pagePostCodeLogs1.table.timeStampOffset'),
        },
        {
          key: 'bootCount',
          label: this.$t('pagePostCodeLogs1.table.bootCount'),
        },
        {
          key: 'postCode',
          label: this.$t('pagePostCodeLogs1.table.postCode'),
        },
        {
          key: 'actions',
          label: '',
  ...
  computed: {
    ...
    allLogs() {
      return this.$store.getters['postCodeLogs1/allPostCodes'].map(
        (postCodes) => {
          return {
            ...postCodes,
            actions: [
              {
                value: 'export',
                title: this.$t('pagePostCodeLogs1.action.exportLogs'),
                enabled: !this.isButtonDisable,
              },
              {
                value: 'download',
                title: this.$t('pagePostCodeLogs1.action.downloadDetails'),
                enabled: !this.isButtonDisable,
              },
            ],
          };
  ...
  created() {
    this.startLoader();
    this.$store.dispatch('postCodeLogs1/getPostCodesLogData').finally(() => {
      this.endLoader();
      this.isBusy = false;
    });
  },
  methods: {
    deleteAllLogs() {
      this.$bvModal
        ...
        .then((deleteConfirmed) => {
          if (deleteConfirmed) {
            this.$store
              .dispatch('postCodeLogs1/deleteAllPostCodeLogs', this.allLogs)
    ...
    exportAllLogsString() {
      const allPostLogData = this.$store.getters['postCodeLogs1/allPostCodes'];
    ...
    exportFileNameByDate(value) {
      ...
      let fileName;
      if (value === 'download') {
        fileName = this.$t('pagePostCodeLogs1.downloadFilePrefix');
      } else if (value === 'export') {
        fileName = this.$t('pagePostCodeLogs1.exportFilePrefix');
      } else {
        fileName = this.$t('pagePostCodeLogs1.allExportFilePrefix');
      }
