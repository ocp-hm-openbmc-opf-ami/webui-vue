<template>
  <div
    ...
    data-test-id="sol-console1-container"
  >
    <access-denied-alert
      v-if="accessDenied"
      :page-name="$t('appPageTitle.serialOverLan1')"
      data-test-id="sol-console1-accessDenied-alert"
    />
    <template v-else>
      <b-row class="d-flex">
        <b-col sm="4" md="6">
          <alert
            ...
            :small="true"
            class="mt-4"
            data-test-id="sol-console1-alert-disconnected"
          >
      ...
      <b-row class="d-flex">
        <b-col class="d-flex flex-column justify-content-end">
          <dl
            ...
            sm="6"
            md="6"
            data-test-id="sol-console1-status-container"
          >
            <dt class="d-inline font-weight-bold mr-1">
              {{ $t('pageSerialOverLan.status') }}:
            </dt>
            <dd class="d-inline" data-test-id="sol-console1-status-text">
        ...
        <b-col v-if="!isFullWindow" class="d-flex justify-content-end">
          <b-button
            ...
            type="button"
            :disabled="disable"
            data-test-id="sol-console1-button-openNewTab"
      ...
      <div id="terminal" ref="panel" data-test-id="sol-console1-terminal"></div>
    </template>
  </div>
</template>
...
export default {
  ...
  watch: {
    consoleWindow() {
      if (this.consoleWindow == false) window.isConsoleWindow1.close();
    },
  },
  created() {
  ...
  mounted() {
    ...
    if (window.isConsoleWindow1) {
      if (window.isConsoleWindow1.closed) {
        this.openTerminal();
      } else {
        this.disable = true;
      }
  ...
  methods: {
    openTerminal() {
      ...
      this.ws = new WebSocket(`wss://${window.location.host}/console1`, [
        token,
      ]);
    ...
    openConsoleWindow() {
      // If isConsoleWindow1 is not null
      ...
      if (window.isConsoleWindow1) {
        ...
        if (!window.isConsoleWindow1.closed) {
          window.isConsoleWindow1.focus();
          return;
        } else {
          this.openNewWindow();
        }
      } else {
        // If isConsoleWindow1 is null, open new window
        this.openNewWindow();
      }
    },
    openNewWindow() {
      ...
      window.isConsoleWindow1 = window.open(
        '#/console/serial-over-lan-console-1',
        '_blank',
