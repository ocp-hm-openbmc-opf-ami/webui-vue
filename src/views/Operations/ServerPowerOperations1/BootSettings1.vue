<template>
  <div class="form-background p-3">
    <b-form novalidate @submit.prevent="handleSubmit">
      <b-form-group
        :label="
          $t('pageServerPowerOperations1.bootSettings.bootSettingsOverride')
        "
      ...
      <b-form-group
        :label="$t('pageServerPowerOperations1.bootSettings.tpmRequiredPolicy')"
      >
        <b-form-text id="tpm-required-policy-help-block">
          {{
            $t(
              'pageServerPowerOperations1.bootSettings.tpmRequiredPolicyHelper',
            )
          }}
        </b-form-text>
        <b-form-checkbox
...
export default {
  ...
  created() {
    this.$root.$emit('server-power-operations1-boot-settings-complete');
  },
  methods: {
    handleSubmit() {
      this.startLoader();
