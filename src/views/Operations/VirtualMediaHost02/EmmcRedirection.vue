...
export default {
  ...
  methods: {
    async getMemoryData() {
      ...
      try {
        const response = await this.$store.dispatch(
          'virtualMediaHost02/getEmmcMemoryData',
        );
    ...
    async getLocalMediaList() {
      try {
        const response = await this.$store.dispatch(
          'virtualMediaHost02/getLocalMediaList',
        );
    ...
    async handleImageSelect(odataId) {
      ...
      try {
        ...
        const response = await this.$store.dispatch(
          'virtualMediaHost02/getImageDetails',
          imageName,
        );
        this.tableItems = [
          {
    ...
    async handleUploadSubmit() {
      ...
      try {
        ...
        await this.$store.dispatch('virtualMediaHost02/uploadLocalMedia', {
          formData: formData,
        });
    ...
    async startRedirection() {
      try {
        await this.$store.dispatch(
          'virtualMediaHost02/startLocalMediaRedirect',
          {
    ...
    async handleStop(item) {
      try {
        if (
          ...
          await this.$store.dispatch(
            'virtualMediaHost02/stopLocalMediaRedirect',
          );
          this.successToast(
    ...
    async handleDelete(item) {
      try {
        if (
          ...
          await this.$store.dispatch(
            'virtualMediaHost02/deleteLocalMedia',
            item.odataId,
          );
